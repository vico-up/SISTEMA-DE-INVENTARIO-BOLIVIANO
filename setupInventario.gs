 ...
 */
function lanzarWebApp() {
  const url = ScriptApp.getService().getUrl();
  const htmlOutput = HtmlService
    .createHtmlOutput('<div style="font-family: Arial; padding: 10px; text-align: center;">' +
      '<h3>La Nube System</h3>' +
      '<p>El sistema se ha actualizado a una experiencia de pantalla completa.</p>' +
      '<a href="' + url + '" target="_blank" style="padding: 10px 20px; background: #2E86AB; color: white; text-decoration: none; border-radius: 5px; display: inline-block; margin-top: 10px;"><b>Abrir Web App</b></a>' +
    '</div>')
    .setWidth(300)
    .setHeight(180);
  SpreadsheetApp.getUi().showModalDialog(htmlOutput, '🌐 Sistema Web');
}
